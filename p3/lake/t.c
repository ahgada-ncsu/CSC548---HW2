...
void run_sim(double *u, double *u0, double *u1, double *pebbles, int n, double h, double end_time)
{
  ...
  #ifdef _OPENACC
    #pragma acc data copy(un[:n*n],uc[:n*n],uo[:n*n],pebbles[:n*n],temp[:n*n])
  #endif
  while(tpdt(&t,dt,end_time))
  {
    ...
    // double *tt;
    // tt = (double*)malloc(sizeof(double) * n * n); 
    ...
    for( i = 0; i < n; i++){
      for( j = 0; j < n; j++){
        ...
        if(j == 0 || j == n - 1 || j==1 || j==n-2){
          ...
          (( uc[idx-1] + uc[idx+1] + uc[n*(n-1)+j] + uc[idx+n] + 
          0.25*(uc[n*(n-1)+j-1] + uc[n*(n-1)+j+1] + uc[idx+n-1] + uc[idx+n+1]) +
          0.125*(uc[idx-1-1] + uc[idx+1+1] + uc[n*(n-2)+j] + uc[idx+n+n]) -
          5.5 * uc[idx])/(h * h) + f(pebbles[idx],t));
        }else if(i==1){
          un[idx] = 2*uc[idx] - uo[idx] + VSQR *(dt * dt) *
          (( uc[idx-1] + uc[idx+1] + uc[idx-n] + uc[idx+n] + 
          0.25*(uc[idx-n-1] + uc[idx-n+1] + uc[idx+n-1] + uc[idx+n+1]) +
          0.125*(uc[idx-1-1] + uc[idx+1+1] + uc[n*(n-1)+j] + uc[idx+n+n]) -
          5.5 * uc[idx])/(h * h) + f(pebbles[idx],t));
        }else if(i==n-2){
          un[idx] = 2*uc[idx] - uo[idx] + VSQR *(dt * dt) *
          (( uc[idx-1] + uc[idx+1] + uc[idx-n] + uc[idx+n] + 
          0.25*(uc[idx-n-1] + uc[idx-n+1] + uc[idx+n-1] + uc[idx+n+1]) +
          0.125*(uc[idx-1-1] + uc[idx+1+1] + uc[idx-n-n] + uc[j]) -
          5.5 * uc[idx])/(h * h) + f(pebbles[idx],t));
        }else if(i==n-1){
          un[idx] = 2*uc[idx] - uo[idx] + VSQR *(dt * dt) *
          (( uc[idx-1] + uc[idx+1] + uc[idx-n] + uc[j] + 
          0.25*(uc[idx-n-1] + uc[idx-n+1] + uc[j-1] + uc[j+1]) +
          0.125*(uc[idx-1-1] + uc[idx+1+1] + uc[idx-n-n] + uc[n+j]) -
          5.5 * uc[idx])/(h * h) + f(pebbles[idx],t));
        }else{
          un[idx] = 2*uc[idx] - uo[idx] + VSQR *(dt * dt) *
          (( uc[idx-1] + uc[idx+1] + uc[idx-n] + uc[idx+n] + 
    ...
    // pointer optimization to avoid memcpy.
    temp = uo;
    uo = uc;
    uc = un;
    un = temp;
