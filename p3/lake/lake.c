...
void run_sim(double *u, double *u0, double *u1, double *pebbles, int n, double h, double end_time)
{
  ...
  // Added ACC pragma to copy data to the device in the beginning of the loop
  #ifdef _OPENACC
    #pragma acc data copy(uc[:n*n],uo[:n*n],pebbles[:n*n], temp[:n*n], un[:n*n])
  #endif
  while(tpdt(&t,dt,end_time))
  {
    ...
    // collapsing manually and parallelizing both loops in openMP
    // note that the conditions handle north-south wrapping in the heatmap
    #ifdef _OPENMP
      #pragma omp parallel collapse(2) for private(i,j,idx) num_threads(nthreads)
    #endif
    // parallelizing the loop in openACC
    #ifdef _OPENACC 
      #pragma acc parallel loop present(uc,uo,un,pebbles,temp)
    #endif
    for(i=0; i<n; i++){
      for(j=0; j<n; j++){
        idx = j + i * n;
        if(i == 0 || i == n - 1 || i==1 || i==n-2){
          un[idx] = 0.;
        }else if(j==0){
          un[idx] = 2*uc[idx] - uo[idx] + VSQR *(dt * dt) *
          (( uc[idx-1] + uc[idx+1] + uc[n*(n-1)+i] + uc[idx+n] + 
          0.25*(uc[n*(n-1)+i-1] + uc[n*(n-1)+i+1] + uc[idx+n-1] + uc[idx+n+1]) +
          0.125*(uc[idx-1-1] + uc[idx+1+1] + uc[n*(n-2)+i] + uc[idx+n+n]) -
          5.5 * uc[idx])/(h * h) + f(pebbles[idx],t));
        }else if(j==1){
          un[idx] = 2*uc[idx] - uo[idx] + VSQR *(dt * dt) *
          (( uc[idx-1] + uc[idx+1] + uc[idx-n] + uc[idx+n] + 
          0.25*(uc[idx-n-1] + uc[idx-n+1] + uc[idx+n-1] + uc[idx+n+1]) +
          0.125*(uc[idx-1-1] + uc[idx+1+1] + uc[n*(n-1)+i] + uc[idx+n+n]) -
          5.5 * uc[idx])/(h * h) + f(pebbles[idx],t));
        }else if(j==n-2){
          un[idx] = 2*uc[idx] - uo[idx] + VSQR *(dt * dt) *
          (( uc[idx-1] + uc[idx+1] + uc[idx-n] + uc[idx+n] + 
          0.25*(uc[idx-n-1] + uc[idx-n+1] + uc[idx+n-1] + uc[idx+n+1]) +
          0.125*(uc[idx-1-1] + uc[idx+1+1] + uc[idx-n-n] + uc[i]) -
          5.5 * uc[idx])/(h * h) + f(pebbles[idx],t));
        }else if(j==n-1){
          un[idx] = 2*uc[idx] - uo[idx] + VSQR *(dt * dt) *
          (( uc[idx-1] + uc[idx+1] + uc[idx-n] + uc[i] + 
          0.25*(uc[idx-n-1] + uc[idx-n+1] + uc[i-1] + uc[i+1]) +
          0.125*(uc[idx-1-1] + uc[idx+1+1] + uc[idx-n-n] + uc[n+i]) -
          5.5 * uc[idx])/(h * h) + f(pebbles[idx],t));
        }else{
          un[idx] = 2*uc[idx] - uo[idx] + VSQR *(dt * dt) *
          (( uc[idx-1] + uc[idx+1] + uc[idx-n] + uc[idx+n] + 
    ...
    // pointer optimization to avoid memcpy in openmp and acc. No optimzation for serial.
    #ifdef _OPENMP
      temp = uo;
      uo = uc;
      uc = un;
  ...
  // depending on the parallelization, we need to copy the correct array to the output
  #ifdef _OPENMP
