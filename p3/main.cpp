...
int main(int argc, char** argv) {
  ...
  // Get the event code for the first event
  retval = PAPI_event_name_to_code("PAPI_RES_STL", &event_codes[0]);
  if (retval!= PAPI_OK) {
      fprintf(stderr, "Failed to get event code 1!\n");
      exit(1);
  }

  // Get the event code for the second event
  retval = PAPI_event_name_to_code("PAPI_STL_ICY", &event_codes[1]);
  if (retval!= PAPI_OK) {
      fprintf(stderr, "Failed to get event code 2!\n");
      exit(1);
  }
  ...
  retval= PAPI_add_events(eventset, event_codes, 2);
  if (retval!= PAPI_OK) {
  ...
  if (retval!=PAPI_OK) {
    fprintf(stderr,"[PRS]  Error starting CUDA: %s\n",
      PAPI_strerror(retval));
  }
